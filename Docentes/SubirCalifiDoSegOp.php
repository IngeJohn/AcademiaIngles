...
function numeros($idgrupo){
    ...
    $query3 = "SELECT DISTINCT calificaciones.numeroControl FROM calificaciones, gruposasignados, alumnos
    WHERE gruposasignados.idgrupo = calificaciones.idgrupo 
    AND alumnos.numeroControl = calificaciones.numeroControl
    AND calificaciones.idgrupo = alumnos.idgrupoActual
	AND calificaciones.idgrupo = '$idgrupo' AND calificaciones.calificacion <= 69 ORDER BY paterno ASC;";
...
$comentario = "";
    ...
    $query22 = "SELECT alumnos.nombre, alumnos.paterno, alumnos.materno, calificaciones2.calificacion, 
calificaciones2.unidadTema, calificaciones2.id, calificaciones2.comentario, calificaciones2.oportunidad FROM alumnos, calificaciones2 
WHERE alumnos.numeroControl = calificaciones2.numeroControl AND alumnos.numeroControl = '{$num}' AND calificaciones2.idgrupo = '{$idg}';";
...
$query32 = "SELECT promedio2 FROM niveles WHERE numeroControl = '{$num}' AND niveles.idgrupo = '{$idg}';";


$result32 = mysqli_query($link, $query32);
...
while($rowp2 = mysqli_fetch_array($result32)){
    ...
    while($row = mysqli_fetch_array($result2)){
     ...
     while($row2 = mysqli_fetch_array($result22)){
                    ...
                    if($contador22 == 0){
                        ...
                        $data1n2 = $row2[4];
                        $data1i2 = $row2[5];
                        $comentario2 = $comentario2."Unidad ".$row2[4].": ".$row2[6];
                        $data1o2 = $row2[7];
                    }
                    if($contador22 == 1){
                        $data22 = $row2[3];
                        $data2n2 = $row2[4];
                        $data2i2 = $row2[3];
                        $comentario2 = $comentario2."<br>Unidad ".$row2[4].": ".$row2[6];
                        $data2o2 = $row2[7];
                    }
                    if($contador22 == 2){
                        $data32 = $row2[3];
                ...
                <p style='font-size:26px; text-align:center;'>Primera Oportunidad</p>
                ...
                <table  class='table table-sm' style='width:100%'>
                                ...
                                <p style='font-size:26px; text-align:center;'>Segunda Oportunidad</p>
                                ...
                                <table class='table table-sm'  style='width:100%; border: 1px solid gray;'>
                                    ...
                                    <p><b>Promedio Primera Oportunidad: </b>".$p."
                                    <br><b>Promedio Segunda Oportunidad: </b>".$p2."
                                    <br><b>Estado: </b>".$e."
                                    <br><b>Tipo de Calificación:  </b>".$o."
                                    <br><b>Comentario final:</b></p>
                                <table  class='table table-sm' style='width:100%; border: 1px solid gray;'>
...
function calificar($numcon, $unipost, $califipost, $comen, $opor, $idg){      
    ...
    if(empty(trim($_POST["unidadTema"]))){
        ...
        $sql = "SELECT unidadTema FROM calificaciones2 WHERE numeroControl = '{$numcon}' AND idgrupo = '{$idg}' AND unidadTema = ?;";
    ...
    if(empty($unidadTema_err1) && empty($calificacion_err1)){
        ...
        $sql = "INSERT INTO calificaciones2 ( numeroControl, unidadTema, calificacion, comentario, oportunidad, idgrupo) VALUES (?,?,?,?,?,?);";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                        ...
                        if ($stmtp = $link->prepare("UPDATE niveles SET promedio2 = 0.0 WHERE numeroControl = '{$numcon}' AND idgrupo = '{$idg}'")) {
                            $stmtp->execute();
                ...
                header("location: SubirCalifiDoSegOp.php");
                exit();
            } else{
...
function modificar($numcon, $unipost, $califipost, $comen, $opor, $idg){      
    ...
    if(empty($unidadTema_err2) && empty($calificacion_err2)){
        ...
        // Prepare an update statement
        $sql = "UPDATE calificaciones2 SET calificacion = ?, comentario = ?, oportunidad = '2aOp' WHERE unidadTema = ? AND numeroControl = ? AND idgrupo = ?;";
        
        if($stmt = mysqli_prepare($link, $sql)){

            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_calificacion, $param_comentario, $param_unidadTema, $param_numeroControl, $param_idgrupo);
...
function promediar($numcon, $idg , $comen){    
 ...
 global $link;
    ...
    global $param_numeroControl, $param_nivel, $param_parcial, $param_calificacion, $param_idmaestro, $estado;
    
    $promedio = $promedio2 = $promedio3 = 0.0;
    ...
    $contador3 = 0;
        ...
        $consulta = "SELECT  calificacion FROM `calificaciones2` WHERE numeroControl = '{$numcon}' AND idgrupo = '{$idg}';";
        // result1 
   ...
   //=========================================================================================================================================
        ...
        // Prepare a select statement
        $sql = "SELECT ROUND(AVG(calificacion),2) FROM calificaciones WHERE numeroControl = '{$numcon}' AND idgrupo = '{$idg}' AND calificacion >= 70;";
     ...
     //====================================================================================================================================================   
              ...
              // Prepare a select statement
        $sqli = "SELECT ROUND(AVG(calificacion),2) FROM calificaciones2 WHERE numeroControl = '{$numcon}' AND idgrupo = '{$idg}';";
        ...
        $promedio = round(($promedio2+$promedio3)/2,2);
    ...
    // Prepare a select statement
        $sqld = "SELECT id, comentario FROM niveles WHERE numeroControl = '{$numcon}' AND idgrupo = '{$idg}';";
        ...
        if($stmt = mysqli_prepare($link, $sqld)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    ...
                    // Prepare an update statement
                    $sql3 = "UPDATE niveles SET promedio2 = {$promedio}, estado = '{$estado}', comentario = ?, oportunidad = '2aOp' WHERE id = {$id};";
                    ...
                    if($stmt = mysqli_prepare($link, $sql3)){
                        ...
                        // Attempt to execute the prepared statement
                        if(mysqli_stmt_execute($stmt)){
                            echo "<script>alert('Promedio de Segunda Oportunidad modificado con exito.');window.location = ' SubirCalifiDoSegOp.php';</script>";
                    ...
                    // Prepare an update statement
                    $sql = "INSERT INTO niveles(numeroControl, promedio2, estado, comentario, oportunidad, idgrupo) VALUES('{$numcon}','{$promedio}','{$estado}',?,'2aOp','{$idg}');";
                    ...
                    if($stmt = mysqli_prepare($link, $sql)){
                       // echo "si entro";
                        ...
                        if(mysqli_stmt_execute($stmt)){
                            ...
                            echo "<script>alert('Promedio de Segunda Oportunidad registrado con exito.'); window.location = ' SubirCalifiDoSegOp.php';</script>";
                           // header("location: SubirCalifiDo.php");
...
function modales($mod1, $mod2, $mod3, $idmodal1, $idmodal2, $idmodal3, $numcon, $idg){
    ...
    global $unidadTema_err1, $calificacion_err1, $unidadTema_err2, $calificacion_err2, $comentario_err, $promedio, $promedio2, $promedio3, $estado;
    
    
    //============================================================================================================================
    
    $query5 = "SELECT  unidadTema FROM calificaciones WHERE numeroControl = {$numcon} AND idgrupo = '{$idg}' AND calificacion <= 69;";
    ...
    $datarowx = "";
        ...
        while($rowx = mysqli_fetch_array($result5)){      
        
        
        if(isset($rowx[0])){
        
            $datarowx = $datarowx."<option value = '$rowx[0]' > $rowx[0] </option>";
    ...
    $promedio = ( $promedio2 + $promedio3 )/2;
                                    ...
                                    <form action='".htmlspecialchars($_SERVER["PHP_SELF"])."' method='post'> 
                                        <div class='modal-body' style='padding: 0 100px'>
                                            ...
                                            <div class='form-group col-md-6'>
                                                ...
                                                <label>Segunda Oportunidad</label>
                                                <select class='form-control' name= 'oportunidad' >
                                                    <option value ='2aOp' selected> Segunda oportunidad </option>
                                                </select> 
                                ...
                                <form action=' ".htmlspecialchars($_SERVER["PHP_SELF"])."' method='post'> 
                                        <div class='modal-body' style='padding: 0 20px'>
                                            ...
                                            <div class='form-group col-md-12'>
                                                <p><b>Promedio de Segunda Oportunidad: </b>".$promedio."</p>
                ...
                <div class="col-sm-4" style="text-align:center; padding-top:20px; ">
                    ...
                    <div class="col-sm-12" style="text-align: center;">
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <a href="SubirCalifiDoSegOp.php" class="btn btn-outline-light active" role="button" >Segunda Oportunidad</a>
