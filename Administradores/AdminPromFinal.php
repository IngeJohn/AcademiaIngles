...
function grupoID($idg){
    ...
    $nivel = $grupo = $carrera = $modalidad = $id = "";
...
function calificaciones($numCon,$idg){
    ...
    $inicio = $termino = "";
    $libe = $libeTipo = $comen = $fech = $peri  = $id = "" ;
    $unitem = $califi = $idparfe = "";
    $tu = 0;
    $prom = 0.0;
    $sum = 0.0;
    $op = '1aOp';
    ...
    if ($stmtg = $link->prepare("SELECT fechaInicio, fechaTermino FROM parcialfechas WHERE numeroParcial = 1")) {
        ...
        $stmtg->close();
            ...
            if ($stmt3 = $link->prepare("SELECT idparcialFechas FROM parcialfechas WHERE numeroParcial = 1")) {
            ...
            if ($stmtk = $link->prepare("SELECT  liberacion, liberacionTipo, comentario, promedioFinal, fecha, periodo, id FROM liberaciones WHERE numeroControl = '{$numCon}'")) {
                
                $stmtk->execute();

                /* bind variables to prepared statement */
                $stmtk->bind_result($libe,$libeTipo,$comen,$califi,$fech,$peri, $id);
    ...
    $query = "SELECT gruposasignados.nivel, niveles.promedio, niveles.oportunidad, niveles.estado 
                        FROM niveles, gruposasignados 
                        WHERE niveles.idgrupo = gruposasignados.idgrupo 
                        AND niveles.numeroControl = '$numCon' ORDER BY gruposasignados.nivel ASC";
        ...
        if(isset($row[3])){
            ...
            if($row[3]=='No acreditado'){
                
                $ac = 'No Acredita';
            
            }elseif($row[3]=='En curso'){ 
...
if(isset($_REQUEST['action']))
{
	switch($_REQUEST['action'])
	{
		case 'actualizar':
            ...
            $alm->__SET('liberacionTipo',        $_REQUEST['liberacionTipo']);
            $alm->__SET('fecha',                 $_REQUEST['fecha']);
            $alm->__SET('comentario',            $_REQUEST['comentario']);
            $alm->__SET('promedioFinal',         $_REQUEST['promedioFinal']);
            $alm->__SET('id',                    $_REQUEST['id']);
			$alm->__SET('periodo',               $_REQUEST['periodo']);
            $alm->__SET('puntaje',               $_REQUEST['puntaje']);

			$model->Actualizar($alm);
			header('Location: AdminPromFinal.php');
			break;
            ...
            <div class="row">
                    ...
                    <div class="col-sm-12" style="text-align: center;">
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <a href="AdminPromFinal.php" class="btn btn-outline-light active" role="button" >Promedio Final y Liberación</a>
              ...
              <div>  
                <form action="?action=<?php echo $alm->id > 0 ? 'actualizar' : 'registrar'; ?>" method="post" onsubmit="return confirm('Presiona OK para continuar.');">
                    ...
                    <table class="table table-bordered table-dark table-sm"  >
                        ...
                        <tr>
                            <th colspan="4" style="text-align:center;">Calificaciones Todos los Niveles</th>
                        </tr>
                        
                        <?php if($alm->__GET('numeroControl')==''){}else{ echo calificaciones($alm->__GET('numeroControl'),$alm->__GET('idgrupo')); }   ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <input id="datepicker" width="240" name="fecha" class="form-control">
                                    <script>
                                        ...
                                        $('#datepicker').val('<?php echo $alm->__GET('fecha'); ?>');
                                    </script> 
                            </td>
                            <td>
                                <input type="text" name="periodo" class="form-control" value="<?php echo $alm->__GET('periodo'); ?>" required> 
                            </td>
                        </tr>
                        <tr>
                            <th>Liberación</th>
                            <th>Tipo de Liberación</th>
                            <th>Calificacíon Final</th>
                            <th>Puntaje</th>
                        </tr>
                        <tr>
                            <td>
                                <select class="custom-select custom-select mb-3" name="liberacion" required>
                                  <option value="" selected>Elige...</option>
                                  <option value="Liberado" <?php if($alm->__GET('liberacion')==='Liberado'){echo "selected";}else{echo "";} ?>>Liberado</option>
                                  <option value="No liberado" <?php if($alm->__GET('liberacion')==='No liberado'){echo "selected";}else{echo "";} ?>>No liberado</option>
                                  <option value="" <?php if($alm->__GET('liberacion')==NULL){echo "selected";}else{echo "hidden";} ?>>Sin Asignar</option>

                                </select>
                            </td>
                            <td colspan="1" >
                                <select class="custom-select custom-select mb-3" name="liberacionTipo">
                                  <option value="" selected>Elige...</option>
                                   <?php foreach($model->Listar0() as $r): ?>
                                  <option value="<?php echo $r->__GET('liberacion'); ?>" <?php if($r->__GET('liberacion') == $alm->__GET('liberacionTipo')){echo "selected";}else{echo "";} ?>><?php echo $r->__GET('liberacion'); ?></option>
                                  <?php endforeach; ?>
                                </select>
                            </td>
                            <td>
                                <input type="text" name="promedioFinal" class="form-control" value="<?php echo $alm->__GET('promedioFinal'); ?>" required> 
                            </td>
                            <td>
                                <input type="text" name="puntaje" class="form-control" value="<?php echo $alm->__GET('puntaje'); ?>" > 
                            </td>
                        </tr>
                         ...
                         <tr>
                             <td colspan="2">
                                <textarea class="form-control" rows="3" id="comentario" name="comentario" placeholder="Comentario"><?php  echo $alm->__GET('comentario');?></textarea> 
                             </td>
                            <td colspan="2" style="text-align:right;">
                                <a href="AdminPromFinal.php" class="btn btn-light btn-sm">Limpiar campos</a> &nbsp;&nbsp;&nbsp;
